 ...
 * @author: zeting
 * @date: 2022/7/28
 * 文本 自由选择TextView文本的帮助类
 */
public class SelectableTextHelper {
    private static final String TAG = SelectableTextHelper.class.getSimpleName();
    /**
     * 默认选择长度
     */
...
//    private int mWindowViewHeight;
     ...
     * 设置文本选择监听器
     */
    private SelectActionListener mSelectListener;
...
//        this.mWindowView = builder.mWindowView;
        ...
        this.mSelectListener = builder.mSelectListener;
        init();
    }
            ...
            public boolean onTouch(View v, MotionEvent event) {
                switch (event.getAction()) {
                    case ACTION_DOWN:
                        ...
                        Log.d(TAG, "mtext:" + mTextView.getText().toString() + "  touch x:" + mTextViewDownPointF.x + " y:" + mTextViewDownPointF.y);
                        closeSelectView();
                        break;
                    default:
                        break;
        ...
        mTextView.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if (mEnable) {
                    ...
                    Log.d(TAG, "mtext:" + mTextView.getText().toString() + " x:" + x + " y:" + y + " touch x:" + mTextViewDownPointF.x + " y:" + mTextViewDownPointF.y);
                    showSelectView(x, y);
                }
                return true;
            }
...
//        });
    ...
    private void showSelectView() {
        ...
        if (layout == null || endLine > layout.getLineCount() || startOffset > layout.getLineCount()) {
            return;
        }
    ...
    private void showSelectView(final float x, final float y) {
        ...
        if (mOperatePopupWindow == null) {
            mOperatePopupWindow = new OperatePopupWindow(mContext);
        }
    ...
    private void selectText(int startOffset, int endOffset) {
        ...
        if (mSelectListener != null) {
            mSelectListener.onTextSelected(mSelectionInfo.content);
        }
    }
...
//        Runtime.getRuntime().gc();
     ...
     */
    public void setOnSelectedListener(@NonNull SelectActionListener onSelectedListener) {
    ...
    private class OperatePopupWindow {
        ...
        public OperatePopupWindow(final Context context) {
            ...
            mWindowViewWidth = contentView.getMeasuredWidth();
            mWindowViewHeight = contentView.getMeasuredHeight();

            contentView.findViewById(R.id.tv_copy).setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (mSelectListener != null) {
                        mSelectListener.onTextSelectedCopy(getSelectedCharSequence());
                    }
                    close();
                }
            });
            ...
            contentView.findViewById(R.id.tv_reply).setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mSelectListener != null) {
                        mSelectListener.onTextSelectedReply(getSelectedCharSequence());
                    }
                    close();
                }
            });
        ...
        public void show() {
            ...
            mTextView.getWindowVisibleDisplayFrame(visibleWindowFrame);
            int[] anchorLocation = new int[2];
            mTextView.getLocationOnScreen(anchorLocation);
            x = anchorLocation[0] + (mTextView.getWidth() / 2) - (mWindowViewWidth / 2);
            y = anchorLocation[1] - (mTextView.getHeight() / 2);

            Log.e(TAG, "visibleWindowFrame w:" + visibleWindowFrame.width() + " h:" + visibleWindowFrame.height() + " w:" + mTextView.getWidth() + " h:" + mTextView.getHeight());

            if (mStatus == END_CURSOR_SHOWED || mStatus == ALL_HIDED || mStatus == ALL_CLOSED) {
                return;
            }
            Log.w(TAG, "PopupWindow位置 x：" + x + " - y:" + y + " -" + mTextView.getText());
            mPopupWindow.showAtLocation(mTextView, Gravity.NO_GRAVITY, x, y);
        }
    ...
    public static class Builder {
        ...
        public Builder(@NonNull TextView textView) {
            ...
            this.mCursorHandleColor = mTextView.getContext().getResources().getColor(R.color.color_3D56FF);
            this.mCursorHandleSizeInDp = 20;
            this.mSelectedColor = mTextView.getContext().getResources().getColor(R.color.color_f3f4f7);
        }
...
//            return this;
//        }
        public Builder addSelectActionListener(SelectActionListener listener) {
