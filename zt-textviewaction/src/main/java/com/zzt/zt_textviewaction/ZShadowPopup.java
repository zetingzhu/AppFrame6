 ...
 * @author: zeting
 * @date: 2021/12/2
 * PopupWindow 背景是带上阴影的
 */
public class ZShadowPopup extends ZBasePopup {
    private static final String TAG = ZShadowPopup.class.getSimpleName();
    ...
    private Drawable mBgUsedColorGradient;    // 背景渐变背景
    ...
    //是否隐藏阴影
    private boolean isHiddenShadow = false;
    private int mShadowColor; // 阴影颜色
    private int mShadowLimit;// 阴影宽度
    // 有偏移是否对称
    private boolean isSym;

    public ZShadowPopup(Context context, int width, int height) {
        super(context);
        mInitWidth = width;
        mInitHeight = height;
    }

    public ZShadowPopup arrow(boolean showArrow) {
        mShowArrow = showArrow;
        return this;
    }

    public ZShadowPopup arrowSize(int width, int height) {
        mArrowWidth = width;
        mArrowHeight = height;
        return this;
    }

    public ZShadowPopup radius(int radius) {
        mRadius = radius;
        return this;
    }

    public ZShadowPopup edgeProtection(int distance) {
    ...
    public ZShadowPopup edgeProtection(int left, int top, int right, int bottom) {
    ...
    public ZShadowPopup offsetX(int offsetX) {
        mOffsetX = offsetX;
        return this;
    }

    public ZShadowPopup offsetYIfTop(int y) {
        mOffsetYIfTop = y;
        return this;
    }

    public ZShadowPopup offsetYIfBottom(int y) {
        mOffsetYIfBottom = y;
        return this;
    }

    public ZShadowPopup preferredDirection(@ZDirection int preferredDirection) {
        mPreferredDirection = preferredDirection;
        return this;
    }

    public ZShadowPopup view(View contentView) {
        mContentView = contentView;
        return this;
    }

    public ZShadowPopup view(@LayoutRes int contentViewResId) {
        return view(LayoutInflater.from(mContext).inflate(contentViewResId, null));
    }

    public ZShadowPopup borderWidth(int borderWidth) {
        mBorderWidth = borderWidth;
        return this;
    }

    public ZShadowPopup borderColor(int borderColor) {
    ...
    public ZShadowPopup bgColor(int bgColor) {
        mBgUsedColor = bgColor;
        return this;
    }

    public ZShadowPopup bgColorGradient(int bgColors[]) {
        mBgUsedColorGradient = new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT, bgColors);
        return this;
    }

    public ZShadowPopup bgColorDrawable(Drawable bgDrawable) {
        mBgUsedColorGradient = bgDrawable;
        return this;
    }
    ...
    public void show(@NonNull View anchor) {
        ...
        mWindow.setWidth(showInfo.windowWidth() + mShadowLimit * 2);
        mWindow.setHeight(showInfo.windowHeight() + mShadowLimit * 2);
    ...
    private void decorateContentView(ShowInfo showInfo) {
         ...
         * 设置背景阴影的东西
         */
        ZShadowLayout shadowLayout = new ZShadowLayout(mContext);
        shadowLayout.setShadowHidden(isHiddenShadow);
        shadowLayout.setShadowColor(mShadowColor);
        shadowLayout.setShadowLimit(mShadowLimit);
        shadowLayout.setShadowSymmetry(isSym);
        shadowLayout.addView(decorRootView);

        mWindow.setContentView(shadowLayout);
    }
        ...
        protected void dispatchDraw(Canvas canvas) {
            ...
            if (mShowArrow) {
                if (mShowInfo.direction == ZDirection.DIRECTION_TOP) {
                    ...
                    int l = mShowInfo.anchorCenter - mShowInfo.x - mArrowWidth / 2 + mShadowLimit * 2;
                    l = Math.min(Math.max(l, mShowInfo.decorationLeft), getWidth() - mShowInfo.decorationRight - mArrowWidth + mShadowLimit * 2);
